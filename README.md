...
flowchart TD
    ...
    E --> F["MVP 实验"]
    F --> G["progress-review<br/>进度与阻塞检查"]
    G --> H["weekly-review<br/>周复盘与轨道判断"]
    H -->|"证据足够"| I["写作与投稿准备"]
    H -->|"证据不足"| E
    I --> J["context-companion<br/>保存状态与下一轮提示"]
```
...
| 实验阶段 | `experiment-memory-scout` 防止重复实验，`progress-review` 检查阻塞 | 暂不需要，除非要设计图或论文故事 |
