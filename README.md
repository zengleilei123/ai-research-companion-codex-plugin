...
```mermaid
flowchart TD
    A["新研究项目 / 新 idea"] --> B["project-onboarding<br/>项目设定"]
    ...
    I -->|"证据足够"| J["写作与投稿准备"]
    I -->|"证据不足"| E
    J --> K["context-companion<br/>保存状态与下一轮提示"]
```
...
| 阶段 | 本插件负责 | 可选外部 skill |
| --- | --- | --- |
| 新项目启动 | `project-onboarding` 明确问题、资源、baseline、MVP | 暂不需要 |
...
| 实验阶段 | `experiment-memory-scout` 防止重复实验，`training-monitor` 监控训练健康，`progress-review` 检查阻塞 | 暂不需要，除非要设计图或论文故事 |
...
当前版本新增 `training-monitor`，用于把训练日志、指标、checkpoint 和资源状态变成研究决策信号。它适合监控：
